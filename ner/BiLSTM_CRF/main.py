        ...
        for epoch in range(opt.max_epoch):
            ...
            for index, batch in enumerate(train_dataloader):
                ...
                if opt.model == 'NERLSTM':
                    loss = criterion(pre_y.view(-1, pre_y.size(-1)), y.view(-1)).mean()
                #CRF
            ...
            for index, batch in enumerate(valid_dataloader):
                ...
                predict = model(val_x)
                if opt.model == 'NERLSTM':
                    loss = criterion(predict.view(-1, predict.size(-1)), val_y.view(-1)).mean()
            ...
            print(report)
            if opt.model == 'NERLSTM':
                torch.save(model.state_dict(), './model/NERLSTM_params.pkl')
            elif opt.model == 'NERLSTM_CRF':
                torch.save(model.state_dict(), './model/NERLSTM_CRF_params.pkl')

    def predict(self, tag, input_str=""):
        if opt.model == 'NERLSTM':
            model.load_state_dict(torch.load("./model/NERLSTM_params.pkl"))
        elif opt.model == 'NERLSTM_CRF':
            model.load_state_dict(torch.load("./model/NERLSTM_CRF_params.pkl"))
        if not input_str:
        ...
        if opt.model == 'NERLSTM':
            paths = torch.argmax(paths,dim = -1).tolist()
